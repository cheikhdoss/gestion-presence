...
@section('content')
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div class="border-t border-gray-200">
            <form action="{{ route('cours.update', $cours) }}" method="POST" class="space-y-6 p-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="matiere" class="block text-sm font-medium text-gray-700">Matière</label>
                    <input type="text" name="matiere" id="matiere" value="{{ old('matiere', $cours->matiere) }}" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $cours->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_heure" id="date_heure" 
                        value="{{ old('date_heure', $cours->date_heure ? date('Y-m-d\TH:i', strtotime($cours->date_heure)) : '') }}" required
                        class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('date_heure')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="salle_id" id="salle_id" required
                        ...
                        @foreach($salles as $salle)
                            <option value="{{ $salle->id }}" {{ old('salle_id', $cours->salle_id) == $salle->id ? 'selected' : '' }}>
                                {{ $salle->libelle }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="professeur_id" id="professeur_id" required
                        ...
                        @foreach($professeurs as $professeur)
                            <option value="{{ $professeur->id }}" {{ old('professeur_id', $cours->professeur_id) == $professeur->id ? 'selected' : '' }}>
                                {{ $professeur->name }}
                            </option>
                        @endforeach
                    </select>
