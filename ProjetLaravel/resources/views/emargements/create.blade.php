...
@section('title', 'Nouvel émargement')

@section('content')
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h1 class="text-2xl font-bold text-gray-900">Nouvel émargement</h1>
        </div>
        <div class="border-t border-gray-200">
            <form action="{{ route('emargements.store') }}" method="POST" class="space-y-6 p-6">
                @csrf
                ...
                <div>
                    ...
                    <select name="cours_id" id="cours_id" required
                        ...
                        @foreach($cours as $course)
                            <option value="{{ $course->id }}" {{ old('cours_id') == $course->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="date_signature" id="date_signature" value="{{ old('date_signature') }}" required
                ...
                <div>
                    ...
                    <textarea name="commentaire" id="commentaire" rows="3"
                        class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('commentaire') }}</textarea>
                    @error('commentaire')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
